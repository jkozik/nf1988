...
fprintf(stderr,"The following months are being processed:\n");
...
for ( monidx=0; monidx<MONTHS; monidx++) {
	...
	fprintf( stderr, "sglf='%s'\n", sglf );
	tabptr->fp = fopen( sglf, "r" ); 
	if( tabptr->fp == NULL ) {
		fprintf( stderr, "fopen('%s') failed.\n", sglf);
		 
		perror( "fopen perror message:  " );
			
	}	
#endif	
	
	if ( (tabptr->fp=fopen(sglf,"r")) != NULL) {
		fprintf(stderr,"%s ",months[monidx]);
	...
	if ( headje == NULL ) {
		for ( i=0; i < MONTHS; i++) {
			...
			if (jefptr->fp != 0 || jefptr->nxtje != NULL || jefptr->line[0] != '\0'){
			   fprintf(stderr,"GLjefile: error found after all je files read.  File-%s.sgl\n",months[i]);
			}
		}
		return (NULL);
	}
