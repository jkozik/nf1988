...
char *ifname,*ofname;
{
	...
	if ( stat(ifname,&sbuf) < 0 ){
		perror("JEglsort: stat cannot open input file");
		return(NULL);
	}
   ...
   inprecords = (char *)malloc((size_t)sbuf.st_size);
	if (inprecords == NULL) {
		fprintf(stderr,"JEglsort:malloc failed to allocate %d bytes\n",sbuf.st_size);
		return(NULL);
	}
	...
	if ((ifp=fopen(ifname,"r")) == NULL) {
		perror("JEglsort: cannot open input file");
		free(inprecords);
		return(NULL);
	}
	...
	while ( fgets(line,SZLINE,ifp) != NULL) {
		...
		if (totsize > sbuf.st_size){
			fprintf(stderr,"JEglsort: Ran out of memory for input file\n");
			fprintf(stderr,"JEglsort: Allocated %d bytes, read %d bytes\n",sbuf.st_size,totsize);
			free(inprecords);
			return(NULL);
		}
		if (rec_cnt+1 >= MAXJEINPREC ) {
			fprintf(stderr,"JEglsort: More than %d input records read\n",MAXJEINPREC);
			free(inprecords);
			return(NULL);
		}
   ...
   if (fclose(ifp) != (int)NULL){
		perror("JEglsort: cannot close input file");
	}

	/* Sort input 
	**  data stored in as records in recptr[]
	*/
   qsort(recptr, rec_cnt, sizeof(char *), (sortFn)JEglscmp);

	/* Read recptr, output to ofname
	*/
	if((ofp=fopen(ofname,"w")) == NULL) {
		perror("JEglsort: cannot open output file");
		free(inprecords);
		return(NULL);
	}
   ...
   if (fclose(ofp) != (int)NULL)
		perror("JEglsort: cannot close output file");
