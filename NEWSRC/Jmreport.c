...
char DElinespage[3];
char DEheadon[2];
GDLIST *GDanal,*GDglrpt,*GDaarpt,*GDstkdtl;
jebool JMheadon;
int JMlinespage;
char JMpsetup[30];
	...
	*searchstr='\0';
	...
	case 'D':
		...
		if (state == analrd)  {
			...
			GDhome(GDglrpt);
			return(glrptdisp);
		}else if (state == stkdtlrd) {
			if (GDstkdtl == (GDLIST *)NULL) {
				GDstkdtl = GDdefine();
				if (GDstkdtl == (GDLIST *)NULL) {
					JMmsg("Display: out of memory");
					return(glrptrd);
				}
				if (GDload(GDstkdtl,"stkdtl.out") == JEFAIL) {
					JMmsg("Display: stkdtl.out not found");
					return(stkdtlrd);
				} 
			}
			GDhome(GDstkdtl);
	...
	case 'R':
		...
		case stkdtlrd:
		   ...
		   leaveok(prompt,FALSE);
			...
			wprmtfld(prompt,"Enter Ticker-",tickerstr);    
			if (tickerstr[0] == KEY_ESC) {
		   		/*
		   		** clear screen
		   		*/
		   		touchwin(body);
	...
	*/
	...
	case 'A':
		/* search for an account number */
		   ...
		   leaveok(prompt,TRUE);
         strcat(searchstr,"^");   /* tell search to anchor search */
