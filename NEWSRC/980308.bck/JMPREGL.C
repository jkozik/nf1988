#include "curses.h"
#include <stdio.h> 
#include <string.h>
#include <ctype.h>
#ifndef turboc
#include <search.h>
#include <direct.h>
#include <sys/types.h>
#endif
#include <stdlib.h>

#include <sys/stat.h>

#include "GLdefs.h"
#include "GLstruct.h"
#include "GLextern.h"

#include "JSdefs.h"
#include "JSstruct.h"
#include "JSextern.h"


/* function templates */
void JEinpchk(char *);

void
JMpregl()
{
	int i;

/* 
** All printing will goto main window.
*/
wclear(body);
wrefresh(body);

/*
** Check for the status of each of the JE input files (00, 01, ..., 12).
*/
JMprtbody("Pregl Starting ...\n\r");
for ( i=0; i < 13; i++){
	(void)JEinpchk(months[i]);
}
JMprtbody("Pregl Completed\n\r");
} /* end of pregl */

void
JEinpchk(jeinpf)
char *jeinpf;
{
	struct stat sbuf1;
	struct stat sbuf2;
	struct stat sbuf3;
	FILE *jeinpfp,*pglfp;
	char pglf[SZPATH];
	char sglf[SZPATH];
	char prtstr[80];
	int srcp,srcs;
	int vrc;

	/* Check status of file
	** For purposes of comments the file name is '00'
	** If 00 does not exist, no processing required.
	*/
	if (stat(jeinpf,&sbuf1) != 0)
		return;  /* if the file does not exist, it's okay. */
	
	/* 
	** Construct file name string for 00.pgl
	*/
	strcpy(pglf,jeinpf);
	strcat(pglf,".pgl");
	srcp = stat(pglf,&sbuf2);
	strcpy(pglf,jeinpf);
	strcat(pglf,".pgl");

	/* 
	** Construct file name string for 00.sgl
	*/
	strcpy(sglf,jeinpf);
	strcat(sglf,".sgl");
	srcs = stat(sglf,&sbuf3);
	strcpy(sglf,jeinpf);
	strcat(sglf,".sgl");


	/*
	** if 00 has been modified since 00.pgl, regenerate 00.pgl and 00.sgl
	** if 00.pgl does not exist and 00 does, generate 00.pgl and 00.sgl
	** if 00.sgl does not exist and 00 does, generate 00.pgl and 00.sgl
	*/
	if (srcp == 0 && sbuf1.st_mtime > sbuf2.st_mtime || srcs || srcp){

		JMprtbody("Processing JE Input File - ");
		JMprtbody(jeinpf);
		JMprtbody("\n\r");

		/* The preceding 'stat' calls assure that the following
		** opens should always be successful, check for fail anyway.
		*/
		vrc=-1;
		if ((jeinpfp=fopen(jeinpf,"r")) == NULL)
			JMprtbody("JE input file cannot open\n\r");

		else if ((pglfp=fopen(pglf,"w")) == NULL)
			JMprtbody("Pregl Output file cannot open\n\r");

		else vrc=JEvalinp(jeinpfp,pglfp);

		/* These closes should never fail, check anyway
		*/
		if (fclose(jeinpfp) == EOF)
			JMprtbody("jeinpfp close failed\n\r");
		if (fclose(pglfp)   ==EOF)
			JMprtbody("pglfp close failed\n\r");

		/* Check JEvalinp error count
		** if >0, then delete 00.pgl file
		*/
		if (vrc > 0) {
			sprintf(prtstr,"pregl: %d errors found, %s file not created\n\r",vrc,pglf);
			JMprtbody(prtstr);
			unlink(pglf); /* ignore if cannot delete */
		} else { /* create  00.sgl */
			if (JEglsort(pglf,sglf) == NULL) {
				sprintf(prtstr,"pregl: sort failed, %d not created\n\r",sglf);
				JMprtbody(prtstr);
				unlink(sglf);
			}
		} /* if "Errors were found validating 00" */

	} /* If "New 00.pgl and 00.sgl are needed" */
}

